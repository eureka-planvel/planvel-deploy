...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('accommodation-form').addEventListener('submit', function (e) {
        ...
        // isHotel 체크 (type 제거된 상태)
        const isHotel = document.querySelector('input[name="isHotel"]:checked');
        if (!isHotel) {
            showToast('호텔 여부를 선택하세요', true);
            return;
        }
        formData.set('isHotel', isHotel.value);  // "true" 또는 "false"

        fetch('/admin-api/accommodation', {
            method: 'POST',
            body: formData,
            credentials: 'include'
        })
        ...
        .catch(error => {
            ...
            showToast(error.message || '숙소 등록 중 오류 발생', true);
        });
    });
});
